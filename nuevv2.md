...
## 🧹 COMANDOS DE LIMPIEZA Y GESTIÓN COMPLETA
...
#### 3. Instalar Argo CD (Opcional - RETO)
...
kubectl rollout status deployment/12-maria-sulca-users-deployment
kubectl rollout status deployment/12-maria-sulca-front-deployment
```

#### Verificar Argo CD (si está instalado)

```bash
# Ver pods de Argo CD
kubectl get pods -n argocd

# Ver servicios de Argo CD
kubectl get svc -n argocd

# Ver applications
kubectl get applications -n argocd

# Describir application
kubectl describe application 12-maria-sulca-app -n argocd
```
...
docker image inspect mariasb/12-maria-sulca-users:latest
docker image inspect mariasb/12-maria-sulca-front:latest
```
...
# Probar frontend (reemplaza IP)
curl -I http://192.168.49.2:30081

# Probar backend (reemplaza IP)
curl http://192.168.49.2:30083/api/v1/users

# Entrar a un pod y verificar variables
kubectl exec -it <nombre-pod-users> -- sh
env | grep SPRING
env | grep BASIC
exit
...
#### 1. Eliminar Recursos Kubernetes (Orden Inverso)

```bash
cd /home/javidev/Escritorio/Demos/k8s

# Eliminar deployments y services
kubectl delete -f 12-maria-sulca-front-service.yml
kubectl delete -f 12-maria-sulca-front-deployment.yml
kubectl delete -f 12-maria-sulca-users-service.yml
kubectl delete -f 12-maria-sulca-users-deployment.yml
kubectl delete -f 12-maria-sulca-secret.yml
kubectl delete -f 12-maria-sulca-configmap.yml

# O eliminar todo de una vez
kubectl delete -f 12-maria-sulca-front-service.yml \
               -f 12-maria-sulca-front-deployment.yml \
               -f 12-maria-sulca-users-service.yml \
               -f 12-maria-sulca-users-deployment.yml \
               -f 12-maria-sulca-secret.yml \
               -f 12-maria-sulca-configmap.yml
```

#### 2. Verificar que Todo se Eliminó

```bash
# Verificar pods (no debe haber ninguno con 12-maria-sulca)
kubectl get pods
...
#### 3. Desinstalar Argo CD (si está instalado)

```bash
# Eliminar application de Argo CD (si existe)
kubectl delete application 12-maria-sulca-app -n argocd
...
#### 4. Eliminar Imágenes Docker Locales (Opcional)
...
docker rmi mariasb/12-maria-sulca-users:latest
docker rmi mariasb/12-maria-sulca-front:latest

# Forzar eliminación si hay contenedores usando la imagen
docker rmi -f mariasb/12-maria-sulca-users:latest
docker rmi -f mariasb/12-maria-sulca-front:latest

# Limpiar imágenes huérfanas y caché
docker image prune -a -f
```

#### 5. Eliminar Imágenes de Docker Hub (Opcional)

```bash
# Esto se hace desde la web de Docker Hub:
# 1. Ve a https://hub.docker.com/
# 2. Login con tu cuenta
# 3. Repositories → mariasb/12-maria-sulca-users → Settings → Delete
# 4. Repositories → mariasb/12-maria-sulca-front → Settings → Delete
```

#### 6. Limpiar Cluster Minikube (Opcional - CUIDADO)

```bash
# Eliminar TODOS los recursos del namespace default
kubectl delete all --all

# O reiniciar Minikube completamente (elimina TODO)
minikube stop
minikube delete
minikube start
...
# Reiniciar frontend
kubectl rollout restart deployment/12-maria-sulca-front-deployment

# Reiniciar ambos
kubectl rollout restart deployment/12-maria-sulca-users-deployment deployment/12-maria-sulca-front-deployment
```

#### Escalar Réplicas

```bash
# Aumentar réplicas backend a 3
kubectl scale deployment/12-maria-sulca-users-deployment --replicas=3

# Reducir réplicas frontend a 1
kubectl scale deployment/12-maria-sulca-front-deployment --replicas=1

# Volver a 2 réplicas
kubectl scale deployment/12-maria-sulca-users-deployment --replicas=2
kubectl scale deployment/12-maria-sulca-front-deployment --replicas=2
```

#### Actualizar Imágenes

```bash
# Reconstruir y pushear nueva versión
docker build -t mariasb/12-maria-sulca-users:v2 .
docker push mariasb/12-maria-sulca-users:v2
...
kubectl rollout undo deployment/12-maria-sulca-users-deployment
```
...
kubectl rollout history deployment/12-maria-sulca-users-deployment

# Ver detalles de una revisión específica
kubectl rollout history deployment/12-maria-sulca-users-deployment --revision=2

# Revertir a una revisión específica
kubectl rollout undo deployment/12-maria-sulca-users-deployment --to-revision=1
```
...
cat > /home/javidev/Escritorio/Demos/k8s/cleanup.sh <<'EOF'
#!/bin/bash
...
kubectl delete -f 12-maria-sulca-front-service.yml 2>/dev/null
kubectl delete -f 12-maria-sulca-front-deployment.yml 2>/dev/null
kubectl delete -f 12-maria-sulca-users-service.yml 2>/dev/null
kubectl delete -f 12-maria-sulca-users-deployment.yml 2>/dev/null
kubectl delete -f 12-maria-sulca-secret.yml 2>/dev/null
kubectl delete -f 12-maria-sulca-configmap.yml 2>/dev/null

# Eliminar Argo CD
echo "🔧 Eliminando Argo CD..."
kubectl delete application 12-maria-sulca-app -n argocd 2>/dev/null
kubectl delete namespace argocd 2>/dev/null

# Eliminar imágenes Docker
echo "🐳 Eliminando imágenes Docker locales..."
docker rmi mariasb/12-maria-sulca-users:latest 2>/dev/null
docker rmi mariasb/12-maria-sulca-front:latest 2>/dev/null
...
chmod +x /home/javidev/Escritorio/Demos/k8s/cleanup.sh
...
cat > /home/javidev/Escritorio/Demos/k8s/deploy.sh <<'EOF'
#!/bin/bash
...
echo "⚙️ Aplicando Deployment backend..."
kubectl apply -f 12-maria-sulca-users-deployment.yml
...
kubectl wait --for=condition=ready pod -l app=12-maria-sulca-users --timeout=120s
kubectl wait --for=condition=ready pod -l app=12-maria-sulca-front --timeout=120s
...
echo "Backend Swagger: http://$MINIKUBE_IP:30083/swagger-ui/index.html"
EOF

# Dar permisos de ejecución
chmod +x /home/javidev/Escritorio/Demos/k8s/deploy.sh
...
* **Orden de eliminación:** Siempre inverso al de creación (Services/Deployments → ConfigMap/Secret)
* **Scripts útiles:** `deploy.sh` para desplegar todo, `cleanup.sh` para limpiar todo
